<template>
     <!-- 顶部位置定位 -->
    <div class="position">
      <span class="iconfont position__locationIcon">&#xe608;</span>
      <span class="position__explain">贵州省贵阳市花果园区中山公关</span>
      <span class="iconfont position__nowIcon">&#xe616;</span>
    </div>
    <!-- 顶部搜索 -->
    <div class="search">
      <span class="iconfont search__icon">&#xe602;</span>
      <div class=" search__text">山姆会员商户名优惠卷</div>
    </div>
    <!-- 顶部轮播图 -->
    <div class="banner">
      <img src="http://www.dell-lee.com/imgs/vue3/banner.jpg" alt="" class="banner__img">
    </div>
    <!-- 图标导航 -->
    <div class="icons">
        <div
        v-for="item in iconsList"
        class="icons__item"
        :key="item"
        >
          <img
            class="icons__item__img"
            :src="`http://www.dell-lee.com/imgs/vue3/${item.imgName}.png`"
          />
          <p class="icons__item__desc">{{item.desc}}</p>
        </div>
    </div>
    <div class="gap"></div>
</template>
<script>

export default {
  name: 'Staticpart',
  setup () {
    const iconsList = [
      { imgName: '超市', desc: '超市便利' },
      { imgName: '菜市场', desc: '菜市场' },
      { imgName: '水果店', desc: '水果店' },
      { imgName: '鲜花', desc: '鲜花绿植' },
      { imgName: '医药健康', desc: '医药健康' },
      { imgName: '家居', desc: '家居时尚' },
      { imgName: '蛋糕', desc: '烘培蛋糕' },
      { imgName: '签到', desc: '签到' },
      { imgName: '大牌免运', desc: '大牌免运' },
      { imgName: '红包', desc: '红包套餐' }
    ]
    return { iconsList }
  }
}
</script>
<style lang="scss">
@import '../../style/viriables.scss';
@import '../../style/mixins.scss';
      //顶部位置定位
  .position{
      position: relative;
      top: 0;
      left: 0;
      width: 100%;
      height: .17rem;
      padding: .16rem 0;
      font-size: .16rem;
      color: $content-fonColor;
      &__locationIcon{
        position:absolute;
        left: 0;
      }
      &__explain{
        width: 2.5rem;
        position:absolute;
        @include sllipsis;
        left: .35rem;
      }
      &__nowIcon{
        position:absolute;
        right: 0;
      }
    }
  //顶部搜索框
  .search{
    margin-bottom:.03rem;
    position: relative;
    border-radius: .16rem;
    background-color: #F5F5F5;
    font-size: .14rem;
    color: #B7B7B7;
    line-height: .32rem;
    .iconfont{
      position: relative;
      top:.02rem;
      padding: 0 0.07rem 0 .16rem;
    }
    &__text{
      display: inline-block;
    }
  }
  //顶部轮播图
  .banner{
    // border:1px solid red;
    height: 0;
    overflow: hidden;
    padding-bottom: 25.4%;
    &__img{
      width: 100%;
    }
  }
  //图标导航
  .icons{
    display: flex;
    flex-wrap: wrap;
    margin-top: .16rem;
    &__item {
      width: 20%;
      &__img {
        display:block;
        width: .4rem;
        height: .4rem;
        margin: 0 auto;
      }
      &__desc {
        margin: .06rem 0 .16rem 0;
        text-align: center;
        color: $content-fonColor;
      }
    }
  }
  .gap {
    margin: 0 -.18rem;
    height: .1rem;
    background: $content-bgColor;
    }
</style>
